# 从自建MySQL迁移至DRDS {#concept_270218 .concept}
...
-   增量数据迁移
    ...
    DTS在全量迁移的基础上读取自建MySQL数据库的binlog信息，将自建MySQL数据库的增量更新数据同步到目标DRDS实例中。通过增量数据迁移可以实现在自建应用不停服的情况下，平滑地完成自建MySQL数据库迁移至DRDS实例。
